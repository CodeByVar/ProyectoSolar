...
def calcular_cobertura_nubes(sol_x, sol_y, nubes):
    ...
    return clamp(cobertura, 0.0, 1.0)
...
def principal():
    ...
    while ejecutando:
        ...
        iluminacion_normalizada = clamp(cosi * atenuacion * (1.0 - cobertura), 0.0, 1.0)
        radiacion_maxima = 800.0
        radiacion_efectiva = radiacion_maxima * iluminacion_normalizada
        potencia = radiacion_efectiva * area_panel * panel_info.get("eficiencia", 0.18)
        ...
        hud_txt = (
            ...
            f"Potencia: {potencia:.1f} W | Ilumin: {iluminacion_normalizada*100:5.1f}% | "
